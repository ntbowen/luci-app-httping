...
* **可视化图表**：集成 ECharts 5.x，提供交互式波形图。
  ...
  * **平滑模式**：采用专业级 EMA (指数移动平均) 算法，平滑过滤噪点的同时保留趋势灵敏度；背景弱化显示原始数据散点，兼顾美观与真实。
  * **暗黑模式**：自动适配浏览器的深色模式。
...
* **v1.1.9**: 优化图表展示逻辑。在平滑模式下，Y 轴范围将根据平滑曲线自动调整并居中显示，彻底排除异常高延迟噪点对视图比例的干扰，使波动趋势一目了然。
* **v1.1.8**: 升级图表算法，采用 EMA (指数移动平均) 替代简单的削峰逻辑。在平滑曲线的同时，背景保留弱化的原始散点数据，完美兼顾趋势展示与异常点排查。
* **v1.1.7**: 优化图表削峰功能，移除不必要的Y轴留白，使削峰后的曲线展示更清晰（具备 Peak cut 效果）。
* **v1.1.6**: 修复 `do_httping` 函数中的正则匹配错误，解决 HTTP 模式显示丢包的问题。
* **v1.1.5**: 修复 `nixio.poll` 兼容性问题，使用数值常量替代符号常量，彻底解决 TCPing 失败的问题。
* **v1.1.4**: 修复 `nixio.poll` 常量引用错误。
* **v1.1.3**: 修复 `nixio.socket` 参数类型错误，解决 Lua 脚本报错退出的问题。
* **v1.1.2**: 修复 init.d 脚本中 command 路径问题，显式调用 Lua 解释器以增强兼容性。
* **v1.1.1**: 修复 Lua 守护进程中的语法错误，导致服务无法启动的问题。
